...
module InfraClass
  module Zentyal
    module Core
      ...
      def zentyal_run_options(new_resource, run_opts = {})
        env = {}
        if new_resource.user
          ...
          env["HOME"] = zentyal_get_homedir(new_resource.user)
        end
      ...
      def zentyal(new_resource, *args, **run_opts)
        zentyal_command = ["zentyal", args].compact.join(" ")
        logger.trace "running #{zentyal_command}"
        shell_out!(zentyal_command, **zentyal_run_options(new_resource, run_opts))
      end
