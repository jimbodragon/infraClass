...
module InfraClass
  module Ops
    module Azure
      ...
      def azure_run_options(new_resource, run_opts = {})
        env = {}
        if new_resource.user
          ...
          env["HOME"] = azure_get_homedir(new_resource.user)
        end
      ...
      def azure(new_resource, *args, **run_opts)
        azure_command = ["azure", args].compact.join(" ")
        logger.trace "running #{azure_command}"
        shell_out!(azure_command, **azure_run_options(new_resource, run_opts))
      end
