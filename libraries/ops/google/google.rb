...
module InfraClass
  module Ops
    module Google
      ...
      def google_run_options(new_resource, run_opts = {})
        env = {}
        if new_resource.user
          ...
          env["HOME"] = google_get_homedir(new_resource.user)
        end
      ...
      def google(new_resource, *args, **run_opts)
        google_command = ["google", args].compact.join(" ")
        logger.trace "running #{google_command}"
        shell_out!(google_command, **google_run_options(new_resource, run_opts))
      end
