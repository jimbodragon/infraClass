...
module InfraClass
  module Ops
    module Docker
      ...
      def docker_run_options(new_resource, run_opts = {})
        env = {}
        if new_resource.user
          ...
          env["HOME"] = docker_get_homedir(new_resource.user)
        end
      ...
      def docker(new_resource, *args, **run_opts)
        docker_command = ["docker", args].compact.join(" ")
        logger.trace "running #{docker_command}"
        shell_out!(docker_command, **docker_run_options(new_resource, run_opts))
      end
