...
module InfraClass
  module Ops
    module OpenStack
      ...
      def openstack_run_options(new_resource, run_opts = {})
        env = {}
        if new_resource.user
          ...
          env["HOME"] = openstack_get_homedir(new_resource.user)
        end
      ...
      def openstack(new_resource, *args, **run_opts)
        openstack_command = ["openstack", args].compact.join(" ")
        logger.trace "running #{openstack_command}"
        shell_out!(openstack_command, **openstack_run_options(new_resource, run_opts))
      end
      ...
      #
      def openstack_get_homedir(user)
