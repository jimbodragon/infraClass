...
module InfraClass
  module Ops
    module Rackspace
      ...
      def rackspace_run_options(new_resource, run_opts = {})
        env = {}
        if new_resource.user
          ...
          env["HOME"] = rackspace_get_homedir(new_resource.user)
        end
      ...
      def rackspace(new_resource, *args, **run_opts)
        rackspace_command = ["rackspace", args].compact.join(" ")
        logger.trace "running #{rackspace_command}"
        shell_out!(rackspace_command, **rackspace_run_options(new_resource, run_opts))
      end
      ...
      #
      def rackspace_get_homedir(user)
