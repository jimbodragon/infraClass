...
module InfraClass
  module Ops
    module AWS
      ...
      def aws_run_options(new_resource, run_opts = {})
        env = {}
        if new_resource.user
          ...
          env["HOME"] = aws_get_homedir(new_resource.user)
        end
      ...
      def aws(new_resource, *args, **run_opts)
        aws_command = ["aws", args].compact.join(" ")
        logger.trace "running #{aws_command}"
        shell_out!(aws_command, **aws_run_options(new_resource, run_opts))
      end
