...
module InfraClass
  module Apps
    module Terraform
      ...
      def terraform_run_options(new_resource, run_opts = {})
        env = {}
        if new_resource.user
          ...
          env["HOME"] = terraform_get_homedir(new_resource.user)
        end
      ...
      def terraform(new_resource, *args, **run_opts)
        terraform_command = ["terraform", args].compact.join(" ")
        logger.trace "running #{terraform_command}"
        shell_out!(terraform_command, **terraform_run_options(new_resource, run_opts))
      end
      ...
      #
      def terraform_get_homedir(user)
