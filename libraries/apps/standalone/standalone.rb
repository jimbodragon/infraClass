...
module InfraClass
  module Apps
    module Standalone
      ...
      def standalone_run_options(new_resource, run_opts = {})
        env = {}
        if new_resource.user
          ...
          env["HOME"] = standalone_get_homedir(new_resource.user)
        end
      ...
      def standalone(new_resource, *args, **run_opts)
        standalone_command = ["standalone", args].compact.join(" ")
        logger.trace "running #{standalone_command}"
        shell_out!(standalone_command, **standalone_run_options(new_resource, run_opts))
      end
      ...
      #
      def standalone_get_homedir(user)
