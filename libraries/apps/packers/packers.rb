...
module InfraClass
  module Apps
    module Packers
      ...
      def packers_run_options(new_resource, run_opts = {})
        env = {}
        if new_resource.user
          ...
          env["HOME"] = packers_get_homedir(new_resource.user)
        end
      ...
      def packers(new_resource, *args, **run_opts)
        packers_command = ["packers", args].compact.join(" ")
        logger.trace "running #{packers_command}"
        shell_out!(packers_command, **packers_run_options(new_resource, run_opts))
      end
