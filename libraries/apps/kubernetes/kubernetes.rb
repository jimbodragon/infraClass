...
module InfraClass
  module Apps
    module Kubernetes
      ...
      def kubernetes_run_options(new_resource, run_opts = {})
        env = {}
        if new_resource.user
          ...
          env["HOME"] = kubernetes_get_homedir(new_resource.user)
        end
      ...
      def kubernetes(new_resource, *args, **run_opts)
        kubernetes_command = ["kubernetes", args].compact.join(" ")
        logger.trace "running #{kubernetes_command}"
        shell_out!(kubernetes_command, **kubernetes_run_options(new_resource, run_opts))
      end
      ...
      #
      def kubernetes_get_homedir(user)
