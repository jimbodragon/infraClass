...
module InfraClass
  module Chef
    module Berks
      ...
      def berks_run_options(new_resource, run_opts = {})
        env = {}
        if new_resource.user
          ...
          env["HOME"] = berks_get_homedir(new_resource.user)
        end
      ...
      def berks(new_resource, *args, **run_opts)
        berks_command = ["berks", args].compact.join(" ")
        logger.trace "running #{berks_command}"
        shell_out!(berks_command, **berks_run_options(new_resource, run_opts))
      end
