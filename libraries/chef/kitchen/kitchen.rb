...
module InfraClass
  module Chef
    module Kitchen
      ...
      def kitchen_run_options(new_resource, run_opts = {})
        env = {}
        if new_resource.user
          ...
          env["HOME"] = kitchen_get_homedir(new_resource.user)
        end
      ...
      def kitchen(new_resource, *args, **run_opts)
        kitchen_command = ["kitchen", args].compact.join(" ")
        logger.trace "running #{kitchen_command}"
        shell_out!(kitchen_command, **kitchen_run_options(new_resource, run_opts))
      end
