...
module InfraClass
  module Chef
    module Knife
      ...
      def knife_run_options(new_resource, run_opts = {})
        env = {}
        if new_resource.user
          ...
          env["HOME"] = knife_get_homedir(new_resource.user)
        end
      ...
      def knife(new_resource, *args, **run_opts)
        knife_command = ["knife", args].compact.join(" ")
        logger.trace "running #{knife_command}"
        shell_out!(knife_command, **knife_run_options(new_resource, run_opts))
      end
